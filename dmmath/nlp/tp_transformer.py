...
import math
import torch
from torch.nn.modules.tp_transformer import TPTransformerDecoder, TPTransformerEncoder, TPTransformerDecoderLayer, \
...
class MultiDecodersTPTransformer(Module):
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 num_decoder_layers=None, dim_feedforward=2048, dropout=0.1,
                 activation="relu"):
        super(MultiDecodersTPTransformer, self).__init__()
        encoder_layer = TPTransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, activation)
        encoder_norm = LayerNorm(d_model)
        self.encoder = TPTransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

        decoders = {}
        if num_decoder_layers:
            for k, v in num_decoder_layers.items():
                decoder_layer = TPTransformerDecoderLayer(d_model, nhead, dim_feedforward, dropout, activation)
                decoder_norm = LayerNorm(d_model)
                decoder = TPTransformerDecoder(decoder_layer, v, decoder_norm)
                decoders[k] = decoder
...
@Model.register("tp-transformer")
class TPTransformer(Model):
    def __init__(
            self,
            vocab: Vocabulary,
        ...
        for tn in self._target_namespaces:
            if tn in self._decoder_namespaces:
                ...
                if self.training and target_tokens:
                    if tn == "answer":
                        tgt, tgt_key_padding_mask = self._encode(self._source_embedder,
                                                                 {"tokens": target_tokens["answer"][:, :-1]})
                    else:
    ...
    def _encode(self, embedder: TextFieldEmbedder, tokens: Dict[str, torch.Tensor]) -> Tuple[
        ...
        src = self._embedding_project_layer(src) * src
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        super(TPTransformer, self).load_state_dict(new_state_dict, strict=False)
